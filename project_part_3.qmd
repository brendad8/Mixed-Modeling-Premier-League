...
The variance for the random effects of squad means measures how much variability there is in the average points scored depending on the team.
...
```
99.6% of the variation in teams intercept is explained by including the goals scored (GF) and goals against (GA) variables. This means that GF and GA accounts for the majority of differences in points scored between teams, which is what we expected to see as teams who scored more goals and defended more goals will have more points.
...
No within team variation in points scored is explained by average team spending, this makes sense as average team spending is a level 2 variable, so when examining an individual team, their average team spending will remain the same across all seasons.
...
We can see that as the goals against the team in that season increases, the points scored for that team in that season decreases. It also appears that the decrease in points for the goals scored against the team is greater if the team has higher scores.
