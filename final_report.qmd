...
(i just put the questions here for now so its easier to answer, definitely need to clean up the format)

Through our modeling process, we were able to determine that the factors that most affect higher or lower point totals in the English Premier league are the number of goals scored, the number of goals scored against them, and average net spending for a club.

We were also able to learn that more money spent in a single season for a club is not associated with earning more points in the following season for that club. However, we saw that more money spent on average across all the seasons for a club is moderately associated with an increase in the average points they earn per season. This finding aligns with our understanding of the league, as stronger clubs with more resources and access to better coaches and players would be expected to spend more on average, but also score more points on average as well.
...
An interesting future step we can consider with future premier league data is to exclude goals from our model entirely, and examine if the other variables will be an accurate predictor of points scored during a season. The benefit to this analysis if the predictions are accurate will be the ability to predict scores for a club in a future season. As our current model stands, we require the results of the season, goals scored and scored against, to be able to make predictions on points earned. However, if we were to only use variables that we can acquire before a season starts, our model may be able to predict results for a season before it begins.
